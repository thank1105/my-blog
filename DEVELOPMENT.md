...
- [x] 相册管理（CRUD）
- [x] 照片批量上传（拖拽多文件）
- [x] 照片编辑（标题、地点、拍摄时间、所属相册）
- [x] 独立照片管理（不属任何相册）
- [x] EXIF 解析（exifr 库提取拍摄时间、相机、镜头等）
- [x] 移动照片到其他相册
- [x] 照片批量删除

**Day 2 - 前台瀑布流**：

- [x] `/photos` 瀑布流总览（CSS columns（避免额外瀑布流依赖））
- [x] `/photos/albums/[slug]` 相册详情瀑布流
- [x] 照片灯箱（复用 Phase 5 的 `Lightbox`）
- [x] 按相册筛选（顶部 tab）
- [x] 瀑布流响应式（4列/3列/2列/1列）
- [x] EXIF 显示（拍摄时间、地点、相机）
...
- [x] 照片上传后 EXIF 自动解析（拍摄时间）
- [x] 瀑布流布局美观（不同高度照片自然排列）
- [x] 点击照片进入灯箱
- [x] 移动端瀑布流自动 1-2 列
- [x] 相册切换流畅
...
- [x] 分类管理（CRUD + 排序 + type 区分）
- [x] 标签管理（CRUD + 合并功能）
- [x] 关联管理（给文章/笔记/作品打标签）
- [x] `/tags` 标签云页面
- [x] `/tags/[slug]` 标签详情页（混合显示文章/笔记/作品）
- [x] `/categories/[slug]` 分类详情页

**Day 2 - 单页内容**：

- [x] 关于我页面编辑器（特殊布局：头像 + 简介 + 社交链接 + 技能 + 时间线）
- [x] Now 页面编辑器（最后更新时间醒目显示）
- [x] Now 页面历史版本（每次更新保存一份）
- [x] 单页管理后台
- [x] 单页前台展示
