// /admin/articles -- list + filter + search (Phase 3 / Day 1).
...
export default async function AdminArticlesPage({ searchParams }: PageProps) {
  ...
  const { rows, total, page, pageSize } = await listArticles(query);
  ...
  return (
    <section className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="font-serif text-2xl font-bold text-ink">文章管理</h1>
      ...
      >
        ...
        <Link href="/admin/articles" className="text-sm text-muted underline-offset-4 hover:underline">
          重置
        </Link>
      </form>
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((row) => <ArticleRowView key={row.id} row={row} />)
            )}
          </tbody>
        </table>
      </div>
...
function ArticleRowView({ row }: { row: ArticleRow }) {
...
function StatusPill({ status }: { status: ArticleRow["status"] }) {
  const cls =
    status === "PUBLISHED"
...
function VisibilityPill({ visibility }: { visibility: ArticleRow["visibility"] }) {
...
// Force the symbol import above to be referenced (avoids tree-shaking).
void statusValues;
void visibilityValues;
