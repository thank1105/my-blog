...
/**
 * Home page (Phase 2 / Day 1 skeleton).
 *
 * Phase 0 landed a one-line 「PHASE 0 · FOUNDATION」card here.
 * Phase 2 wires the front-end chrome — Header + Footer — onto the
 * `/` route so the chrome is visible end-to-end. The article/project/
 * note/photo streams themselves arrive in Phases 3-6; today'\''s data
 * below is intentionally hard-coded so the layout can be reviewed
 * without the database tables being populated.
 *
 * Why does this page render Header/Footer manually instead of relying
 * on `(frontend)/layout.tsx`? Because (frontend) currently owns only the
 * `/login` route, which deliberately has no chrome. The proper
 * `(frontend)` route group layout (with mobile responsive behaviour,
 * 404 / error / loading pages) ships on Day 2; today we just want a
 * visible skeleton to demo Day 1.
 *
 * `buttonVariants` is reused via `<Link>` so the primary CTA stays a
 * real anchor (good for SEO / right-click / middle-click) without
 * introducing `@radix-ui/react-slot` for one button.
 */
